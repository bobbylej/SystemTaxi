DashboardModule.controller( 'MapController', ['$scope', '$http', '$filter', 'toastr', function( $scope, $http, $filter ) {
  ...
  //show map
  //AIzaSyC9R7KitA09oH9BUFBGAykDas2y4WUNyPY
  $.getScript( 'http://maps.googleapis.com/maps/api/js?key=AIzaSyC9R7KitA09oH9BUFBGAykDas2y4WUNyPY', function( data, textStatus, jqxhr ) {
    ...
    function initialize() {
      ...
      console.log( 'MAPS2', map );
    }
    initialize();
    //show taxi on maps
    getWorkTaxi( [], function() { makeTaxiMarkers(); } );
    //show courses addresses on map
    getActualCourses( [], function() { makeAddressMarkers( allCourses ); } );
  });
/*
  function initialize() {
    var mapProp = {
...
*/
  ...
  function getSingleAddressMarker( course ) {
    ...
    $.getJSON( 'http://maps.googleapis.com/maps/api/geocode/json?address='+addressStart+'&sensor=false', null, function (data) {
        ...
        coursesMarkers[ course.id ].taxi = course.taksowkarz;
        coursesMarkers[ course.id ].client = course.klient;
